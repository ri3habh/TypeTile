...
app.get(
  ...
  asyncWrapper(async (req, res) => {
    let topTenNormal = [],
      ...
      topTenRandom = [],
      topTenRandomPoison = [];
    const normalLeaderboard = await Leaderboard.findOne({
      name: "normal",
    }).populate({
      path: "usersAndScores",
      populate: {
    ...
    const normalPoisonLeaderboard = await Leaderboard.findOne({
      name: "normalPoison",
    }).populate({
      path: "usersAndScores",
      populate: {
    ...
    const randomLeaderboard = await Leaderboard.findOne({
      name: "random",
    }).populate({
      path: "usersAndScores",
      populate: {
    ...
    const randomPoisonLeaderboard = await Leaderboard.findOne({
      name: "randomPoison",
    }).populate({
      path: "usersAndScores",
      populate: {
...
app.get("/game", (req, res) => {
  ...
  res.render("game", {
    randSentences: generateRandomSentences(100),
    mode,
    poison,
    font,
    initialSpeed,
...
const quickSortByScore = (users) => {
  ...
  let less = users.filter((elem) => {
    elem.highScore < pivot;
  });
  let greater = users.filter((elem) => {
    elem.highScore >= pivot;
  });
